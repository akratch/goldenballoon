 ...
 * exact residual from the registered transform onto the interpolated pose, so
 * the current endpoint reconstructs the registered source exactly.
 */
bool mdkr_camera_replay_object_world(
    ...
    transform.x_position =
        target.position[0] + (owner->source_position[0] - current.position[0]);
    transform.y_position =
        target.position[1] + (owner->source_position[1] - current.position[1]);
    transform.z_position =
        target.position[2] + (owner->source_position[2] - current.position[2]);
    transform.rotation.y_rotation = (s16)(
        target.rotation_y +
        (s16)(owner->source_rotation[0] - current.rotation_y));
    transform.rotation.x_rotation = (s16)(
        target.rotation_x +
        (s16)(owner->source_rotation[1] - current.rotation_x));
    transform.rotation.z_rotation = (s16)(
        target.rotation_z +
        (s16)(owner->source_rotation[2] - current.rotation_z));
    if (current.scale != 0.0f) {
        transform.scale =
            target.scale * (owner->source_scale / current.scale);
    } else {
        transform.scale =
            target.scale + (owner->source_scale - current.scale);
    }
...
static bool mdkr_camera_replay_object_transform(
    ...
    out->rotation.z_rotation = (s16)(
        ...
        (s16)(owner->source_rotation[2] - current.rotation_z));
    if (current.scale != 0.0f) {
        out->scale = target.scale * (owner->source_scale / current.scale);
    } else {
        out->scale = target.scale + (owner->source_scale - current.scale);
    }
    return true;
...
bool mdkr_camera_replay_effect_world(
    ...
    presentation_lerp3(previous->effect_position, current->effect_position,
                       numerator, denominator, &effect.x_position);
