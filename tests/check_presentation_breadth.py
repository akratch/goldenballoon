...
hash comparison at all:
  ...
  * retained point/line particle batches, compatible pairs, and changed XYZ;
    the battle-challenge arm is the positive witness because line-only content
    correctly produces compatible but stationary meshes;
...
def check_arm(binary: Path, rom: Path, root: Path, arm: Arm, timeout: int,
        ...
        for key in ("particlevertexreg", "particledeformhit",
            ...
            if packet.get(key, 0) <= 0:
                failures.append(
                    ...
                    f"{packet.get(key)} -- particle interpolation is not live "
                    "on its measured positive content")
    ...
    note = (f"{arm.name:20s} {arm.clause:38s} "
            ...
            f"{packet.get('deformoverride', 0)} changed, "
            ...
            f"{packet.get('particledeformhit', 0)} compatible, "
            f"{packet.get('particledeformoverride', 0)} changed")
