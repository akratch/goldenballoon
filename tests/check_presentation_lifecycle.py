...
For each route, original-rate and 60 Hz presentation must produce byte-identical
v3 state, ordered gameplay events, consumed input, and temporary PCM. The live
60 Hz arm must also retire/rebuild replay history without any snapshot, packet,
matrix-freeze, or key-collision failure. A one-row state-hash perturbation is a
same-binary broken-direction control for the comparator.
...
def run(binary: Path, rom: Path, root: Path, scenario: Scenario,
        ...
        extra: dict[str, str] | None = None) -> Result:
...
def validate_pair(scenario: Scenario, baseline: Result,
    ...
    if high.summary.get("interp", 0) <= 0 or high.replay.get("walks", 0) <= 0:
        failures.append(f"{prefix}: 60 Hz arm never replayed an intermediate frame")
    for owner, fields in (
    ...
    with tempfile.TemporaryDirectory(prefix="mdkr-present-lifecycle-") as temp:
        ...
        try:
            for scenario in scenarios:
                ...
                high = run(binary, rom, root, scenario, "60",
                           args.timeout, args.verbose)
                high_results[scenario.name] = high
                failures.extend(validate_pair(scenario, baseline, high))
                notes.append(
                    f"{scenario.name}: {scenario.ticks} fixed ticks, "
                    f"{high.summary.get('presents')} presents, "
                    f"{high.replay.get('walks')} retained walks")
