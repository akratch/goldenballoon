...
int main(void) {
    ...
    /* Discipline: a rate override expires when its evidence dries up. A
     * stale override outliving the display-bound regime is the measured
     * 13 Hz strobe defect (stuck 106 Hz grid under a ~117 Hz panel). */
    {
        MdkrPresentDeadlineClock clock;
        uint64_t now = 0u;
        uint64_t display_next = 0u;
        uint64_t blocked;
        uint64_t target_ns;
        uint64_t display_period = UINT64_C(10416666); /* 96 Hz: force rerate */
        int frame;
        expect("expiry clock init", mdkr_present_deadline_init(&clock, 120u));
        for (frame = 0; frame < 600; frame++) {
            target_ns = mdkr_present_deadline_target(&clock, now);
            if (target_ns > now) now = target_ns;
            blocked = display_next > now ? display_next - now : 0u;
            now += blocked;
            display_next += display_period;
            mdkr_present_deadline_feedback(&clock, blocked, 0, now);
        ...
        expect("expiry precondition: the grid re-rated",
               mdkr_present_deadline_period_ns(&clock) !=
                   UINT64_C(1000000000) / 120u);
        /* Panel returns to fast retirement: acquire never blocks again, so
         * the phase creep is never punished. Each two-periods'-worth of
         * unpunished pull decays the override half of its gap; after
         * enough blockless frames it must be home on the exact grid.
         * 2 periods / CREEP per frame ~ 840 frames per step; 14 steps
         * with margin. */
        for (frame = 0; frame < 840 * 12; frame++) {
            now += UINT64_C(8333333);
            mdkr_present_deadline_feedback(&clock, 0u, 0, now);
            mdkr_present_deadline_commit(&clock, now);
        }
        expect("unpunished early pull decays the stale override home",
               mdkr_present_deadline_period_ns(&clock) ==
                   UINT64_C(1000000000) / 120u);
        expect("the decay steps were counted",
               clock.rerate_expiries >= 5u && clock.rerate_expiries <= 20u);
    }
