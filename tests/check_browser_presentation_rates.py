#!/usr/bin/env python3
"""Browser rAF schedules preserve fixed-tick authority and honest policy.
...
def run_arm(server: OverlayServer, chrome_path: Path, rom: Path,
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            note = cdp.evaluate(
                "document.querySelector('.quality-note').textContent")
            require(
                isinstance(note, str)
                and "requestAnimationFrame" in note
                and "does not offer an unbounded" in note,
                "browser launcher does not disclose its rAF ceiling",
            )
...
def compare(arm: Arm, result: Result, baseline: Result) -> list[str]:
    ...
    if arm.effective_kind != 0 and result.summary.get("interp", 0) == 0:
        failures.append(f"{arm.name}: no interpolated browser presents")
    ...
    try:
        ...
        for arm in arms:
            ...
            notes.append(
                ...
                f"presents / {result.summary.get('ticks')} fixed ticks")
