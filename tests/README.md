...
`check_webgpu_fault_matrix.py` cross-checks the public 105-point registry
against the executable runtime matrices: all 73 native/shared shipped points
must be named by `check_webgpu_recovery.py`, all three browser-only points by
`check_browser_runtime.py`, and none of the 29 dormant inherited routes may
receive runtime-coverage credit.
...
## Native renderer parity and fallback — `tests/check_renderer_backends.py`
...
and readback constructors reached by DKR. Required resources must rebuild the
device or switch to live GL; mip and diagnostic failures must stay local. Every
...
The check asserts the **exit code** plus the output-overlay ordering contract
...
`MENU_TRACK_SELECT` — mirroring `check_race_multiplayer.py`'s 3P/4P arms. The
...
**Per-player in-race input is separately verified.** Autopilot drives both racers,
which proves split-screen but *not* that controller 1 reaches racer 2. Hold the
throttle on P2 only, with no autopilot, and player 2 drives away (`cp=5`, moved
~5500 world units) while player 1 stays exactly on its grid spot
(`x=-1741.0 z=-6482.0 cp=0`) — so the two pads route to the two racers
independently and are not swapped.
...
```bash
python3 tests/check_determinism.py                    # 3 fixtures, 2 runs each
python3 tests/check_determinism.py --runs 3 --every 200 -v
```
It runs each fixture N times and requires every dumped frame to be byte-identical
across runs. Measured before the fix, `nav_to_character_select` frame 1450 over 10
