...
The 1.0.3 patch release intentionally skips Developer ID signing and
...
## Unsigned 1.0.3 release (default)
...
allowed only when it is exactly `v1.0.3` and that tag resolves to the workflow's
...
1. In the artifact directory, run
   `shasum -a 256 -c Golden-Balloon-1.0.3-macos-arm64-unsigned.dmg.sha256`.
...
`--skip-notarize` path in the workflow. For 1.0.3, that optional artifact is
exactly `Golden-Balloon-1.0.3-macos-arm64-signed-notarized.dmg` and records
