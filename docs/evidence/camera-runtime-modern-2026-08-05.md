...
| Bounded BVH work reports source failure on healthy complex geometry | A 16-chunk fence was conflated with corrupt cache/input | Query work explicitly distinguishes exhaustion. Sphere exhaustion uses the published world AABB as a fail-closed hit and records `sphere_fallbacks`; corruption remains `INVALID` | Adventure route exercises five fallbacks and zero sphere/exact invalid sweeps |
