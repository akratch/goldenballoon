...
bare semantic versions such as `1.5.0`, never `v1.5.0`. The `v` prefix belongs
only to the Git tag. For version 1.5.0, the portable workflow must produce:

- `Golden-Balloon-1.5.0-linux-x86_64.AppImage`
- `Golden-Balloon-1.5.0-linux-x86_64.tar.gz`
...
The exact-manifest `Golden-Balloon-1.5.0-windows-x64.zip` may be attached only
...
### macOS 1.5.0 — unsigned/ad-hoc patch artifact

The public 1.5.0 macOS artifact intentionally skips Developer ID signing and
...
- `Golden-Balloon-1.5.0-macos-arm64-unsigned.dmg`
- `Golden-Balloon-1.5.0-macos-arm64-unsigned.dmg.sha256`
- `Golden-Balloon-1.5.0-macos-arm64-unsigned.dmg.provenance.json`

The provenance sidecar must name that exact DMG, the exact 40-character source
commit, version `1.5.0`, platform `macos`, the DMG SHA-256, and
...
- [ ] `CMakeLists.txt`, `macos/Resources/Info.plist`, the app's `--version`
      output, and the release notes all agree on `1.5.0`.
- [ ] The release commit is the intended `v1.5.0` tag commit. A test artifact
      may omit `release_tag`; an artifact may be published only with
      `release_tag=v1.5.0` resolving to the workflow's exact source commit.
...
- [ ] Build `mdkr64.app` with `--strict-deployment-target`, embed version
      `1.5.0` and the exact source commit, bundle SDL2, then seal nested code
...
After the test artifact passes and `v1.5.0` exists on the exact candidate
commit, publish by dispatching the same source commit with the binding enabled:

```bash
gh workflow run macos-release.yml --ref v1.5.0 \
  -f version=1.5.0 \
  -f trusted_signing=false \
  -f release_tag=v1.5.0
```
...
to the existing `v1.5.0` GitHub Release.

### Optional trusted macOS artifact

The credentialed path is not part of the unsigned 1.5.0 release. If it is used
later, its exact artifact name is
`Golden-Balloon-1.5.0-macos-arm64-signed-notarized.dmg`, with matching
...
Gatekeeper acceptance, and still enforce `release_tag=v1.5.0` against the exact
