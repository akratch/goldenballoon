...
|---|---|---|---|
| Invite capability | Redeem a pending controller or match endpoint | Raw value returned/held client-side only; purpose-HMAC digest until 2 min (controller), 10 min (match), rotation or room close | Never |
...
| SDP/ICE | Establish direct connection; peers and signaling transiently | Forward only; no history/persistence | Never |
...
| Daily reservation aggregate | Reconstruct admitted cost shape; authorized operator only | Twelve fixed operation counts, admitted/tracked units and legacy tracking state in the same 32-day budget shard | Snapshot may be retained in the signed operations ledger; no route, result, identifier or free-form label |
