...
journeys are queued for the dedicated-desktop batch and are not represented as
a fresh pass above.
...
join that same dedicated-desktop evidence batch.

The same evidence boundary applies to the later 2026-08-13 Phone Party
hardening. JavaScript syntax, Python AST, service TypeScript typecheck, focused
low-priority C/C++ compilation and diff hygiene are current. The new QR-failure,
actual-TTL/rotation-generation, malformed/equivocated state, finite reconnect
exact request/media-type and signaling-envelope, native-command,
two-controller targeted-delivery and removal fixtures are authored but have not
been executed on this occupied workstation.
...
invite or dead control surface. The source and browser arms are authored;
behavioral execution remains deferred from this occupied workstation.
...
This entire block is for a dedicated test desktop only. Do not run any command
in it on the occupied maintainer workstation. From the repository root, the
human/operator first supplies the independent desktop attestation:
...
MDKR_DEDICATED_TEST_DESKTOP=1 MDKR_BROWSER_TESTS_ALLOWED=1 python3 tests/check_controller_page.py
MDKR_DEDICATED_TEST_DESKTOP=1 MDKR_BROWSER_TESTS_ALLOWED=1 python3 tests/check_party_host.py
MDKR_DEDICATED_TEST_DESKTOP=1 MDKR_BROWSER_TESTS_ALLOWED=1 python3 tests/check_phone_party_webrtc.py
...
MDKR_DEDICATED_TEST_DESKTOP=1 MDKR_BROWSER_TESTS_ALLOWED=1 python3 tests/check_browser_online_room_gallery.py --build build --shell-dir dist/web
...
MDKR_DEDICATED_TEST_DESKTOP=1 MDKR_BROWSER_TESTS_ALLOWED=1 python3 tests/check_browser_online_two_person.py --shell-dir dist/web
...
MDKR_DEDICATED_TEST_DESKTOP=1 MDKR_BROWSER_TESTS_ALLOWED=1 python3 tests/check_browser_publish_skew.py --shell-dir dist/web
MDKR_DEDICATED_TEST_DESKTOP=1 MDKR_BROWSER_TESTS_ALLOWED=1 python3 tests/check_touch_controls.py --engine-dir build-web --shell-dir dist/web
