...
typedef struct SimSched {
    ...
    unsigned long long accumulator_units; /* ns x field_hz; tick == 2e9 */
    SimSchedStats stats;
} SimSched;
...
/*
 * Same, but in the accumulator's own exact units (ns x field_hz; one tick is
 * 2e9). sim_sched_advance is this function with `elapsed_ns * field_hz`.
 *
